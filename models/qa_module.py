...
from models.sg_module import GCN
...
class ScanQA(nn.Module):
    def __init__(self, num_answers, 
        ...
        self.SG_feat_linear = nn.Sequential(
        ...
        self.sg_fusion = nn.Sequential(
                nn.Linear(384 + mcan_flat_out_size * 2, hidden_size),
                nn.GELU(),
                nn.Dropout(answer_pdrop),
                nn.Linear(hidden_size, mcan_flat_out_size)
        )
    ...
    def forward(self, data_dict):
        ...
        #                                     #
        #######################################
        obj_vecs = data_dict["obj_vecs"]
        pred_vecs = data_dict["pred_vecs"]
        edges = data_dict["edges"]
        obj_vecs, pred_vecs = self.sg_net(obj_vecs, pred_vecs, edges)
        obj_vecs = obj_vecs.mean(1).squeeze(1)
        # print("=====================obj_vecs, pred_vecs", obj_vecs.shape, pred_vecs.shape)
        #######################################
        #                                     #
        #           LANGUAGE BRANCH           #
        #                                     #
        ...
        # print("===================lang_feat object_feat",lang_feat.shape, object_feat.shape, )

        # use obj_vecs: mcan_flat_out_size*2+384
        fuse_feat = self.sg_fusion(torch.cat([lang_feat, object_feat, obj_vecs], dim=-1))
        # fuse_feat = self.sg_fusion(obj_vecs)
        fuse_feat = self.fusion_norm(fuse_feat) # batch, mcan_flat_out_size
        #######################################
        #                                     #
        #           LANGUAGE BRANCH           #
        #                                     #
