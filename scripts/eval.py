...
SCANQA_TRAIN = f"/workspace/UrbanQA/data/qa/{train_mode}/train.json"
...
def get_scanqa(args):
    if args.detection:
        ...
        SCANQA_VAL = f"/workspace/UrbanQA/data/qa/{train_mode}/val_{hop}.json"
        scanqa = SCANQA_TRAIN if args.use_train else SCANQA_VAL
        scanqa = json.loads(open(scanqa, "r").read())
...
def eval_qa(args):
    ...
    if gen_flag:
        ...
        print(f"========saving answer to {score_path} ===========")

    else:
        print(f"loading the scores from {score_path}...")
        with open(score_path, "rb") as f:
            ...
            print(np.mean(answer_acc_at1), np.mean(answer_acc_at10))
